...
Используй черновой план, текст пользователя и найденный контекст (если есть), чтобы предложить конкретный план: какая таблица и какие поля подойдут. 
Выведи:
— Таблица: человекочитаемое название — `db.table`
— Поля (по одному в строку): «человекочитаемое название (column) — краткое описание»
Добавь заметку про период/фильтры и напомни, что можно скорректировать или подтвердить план.
Ответ дай на русском, кратко.
"""
