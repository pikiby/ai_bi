...
- Если пользователь просит ТАБЛИЦУ (а не график) — НЕ добавляй канонические алиасы (date/x/y/value/series). Оставь только человекочитаемые алиасы на русском, без дублей.
- Никогда не дублируй одно и то же выражение в SELECT под разными алиасами. Если уже есть `report_date AS ...`, не добавляй ещё `report_date AS date`; если уже есть `SUM(total_active_users) AS ...`, не добавляй ещё `SUM(total_active_users) AS value`.
