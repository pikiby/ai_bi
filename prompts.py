...
- если в запросе есть слова «график/построить/сделать график» и ранее таблиц нет (в системной справке: «Ранее таблицы не создавались.») → выбери sql;
- если в запросе есть «график…» и таблица уже есть → выбери plotly.
...
- Если в пользовательском запросе явно есть слова «график», «построить график», «сделай график»: — ПОСЛЕ блока ```sql-run``` верни ДОПОЛНИТЕЛЬНО второй блок с исполняемым кодом графика:
  ```plotly-run
  # код на python, создающий fig из df
  ```

Требования к графику:
- Используй ТОЛЬКО столбцы, которые выводит твой SELECT (учти их точные алиасы).
- Разрешены только df, px, go, col(...), has_col(...), COLS — см. системные правила PLOTLY.
- Для «топ N» по городам по сумме оплат — построй bar-chart: x = город, y = сумма оплат (алиас из SELECT), сортировка по убыванию. Не модифицируй df; всё через параметры px/go.
"""
...
RULES_PLOTLY = r"""(RUN-GATING) Политика выполнения графиков:
- Описательные запросы («сделать/построить график …») → возвращай ТОЛЬКО ЧЕРНОВИКИ в нейтральных блоках:
    PLOTLY_DRAFT:
    ```text
    <python/plotly черновик, создаёт fig; не для исполнения>
    ```
    SQL_DRAFT (если уместно):
    ```text
    <черновик SQL; не для исполнения>
    ```
- Явное подтверждение запуска («выполни», «запусти», «построй сейчас», «ок, делай») → используй ИСКЛЮЧИТЕЛЬНО исполняемые блоки:
    ```plotly-run
    # код, создающий переменную fig
    ```
    ```python-run
    # код, создающий переменную fig
    ```
    ...
    -- готовый SQL
    ```
- Не используй ```plotly-run```, ```python-run``` или ```sql``` без суффикса -run, если пользователь не просил запустить прямо сейчас.
...
Если подходящих колонок нет — НЕ генерируй фиктивный код. Верни пустой ответ (без блока ```plotly-run```).
"""