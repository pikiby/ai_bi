...
- rag: явно спрашивают про структуру/описание/алиасы/DDL/каталоги/«какие дашборды/таблицы есть»/нужно прояснить схему.
...
- Запросы вида «какие таблицы доступны в базе», «что есть в базе», «какие дашборды есть» — всегда rag.
...
# 3) РЕЖИМЫ: SQL_PLOTLY (SQL + график в одном ответе)
RULES_SQL_PLOTLY = r"""
РЕЖИМ SQL_PLOTLY:
...
Если задача про числа/агрегации → сгенерируй SQL (см. правила ниже).
Если задача про каталог/описания → выведи структурированный ответ из контекста.

Форматы ответа:

— Каталог/перечень (пример: «какие таблицы доступны/что есть в базе/какие дашборды?»)
Верни Markdown-блок с двумя разделами. Выводи только то, что найдено в контексте.

**Таблицы (доступны для SQL):**
- <db.schema.table> — <краткое описание/назначение>
- ...

**Дашборды (для навигации):**
- <Название> — <URL> — <краткое описание>
- ...

Если в контексте НИЧЕГО не найдено, верни аккуратный fallback (без оправданий):

**Не нашёл описаний таблиц/дашбордов в базе знаний.**
Что можно сделать сейчас:
1) Дайте список таблиц (например, результат `SHOW TABLES FROM <db>`).
2) Или добавьте страницу `docs/schema.md` с перечнем таблиц и полей.
3) Либо укажите, где хранится факт оплат (название БД/схемы/таблицы).

— Числа/метрики/агрегации/топ-N:
Верни РОВНО ОДИН блок:
```sql
SELECT ...
```
Только таблицы/поля из контекста. Если нужных таблиц/полей нет — верни тот же fallback из раздела каталога.
"""
