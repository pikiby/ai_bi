...
Используй черновой план, текст пользователя и найденный контекст (если есть), чтобы предложить конкретный план запроса: какие таблицы, поля, фильтры и период подходят. Перечисли их в удобочитаемом формате, укажи альтернативные поля (если есть) и напомни, что пользователь может уточнить или подтвердить план.
Ответ дай на русском максимум в двух абзацах.
"""
