...
- plotly: просят ПОСТРОИТЬ или ИЗМЕНИТЬ график по df, уже полученному ранее.
...
- "измени цвет на светло серый" → table (если в контексте есть таблица)
- "сделай светло серой" → table (если в контексте есть таблица)
...
- ВАЖНО: Если пользователь просит "изменить цвет" БЕЗ указания "графика" - это может быть про таблицу!
- Если просят СОЗДАТЬ новый график, но в истории уже ЕСТЬ таблица/df (данные получены ранее) → plotly, НЕ sql.
- Если данных ещё нет (нет таблицы/df в истории) → сначала sql для получения данных, затем plotly.
