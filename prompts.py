...
Требования к блоку plotly:

- Используй только df, px, go и helper col(...); не изменяй df и не создавай новые датафреймы.
- Указывай имена столбцов ТОЛЬКО через col("...") (можно перечислить варианты).
- Итогом должен быть объект plotly.graph_objects.Figure в переменной fig.
