...
Определи КАКОЙ ОДИН режим нужен для следующего шага: sql | rag | table | plotly | catalog.
Верни ровно этот блок:

```mode
<sql|rag|table|plotly|catalog>
```
...
- table: просят изменить/отфильтровать/обогатить/переименовать текущую таблицу (df) без нового SQL.
