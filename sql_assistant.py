...
def _build_system_prompt(schema_text: str, rag_appendix: str) -> str:
    ...
    - Если пользователь ЯВНО просит без лимита (например: «без лимита», «все <алиас или название столбца>», «полная таблица», «не добавляй limit») — не добавляй LIMIT.
    - Во всех остальных случаях добавляй LIMIT (например 1000).
