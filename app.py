    ...
    with st.container():
        ...
        st.markdown(
            ...
            ## Ассистент аналитики: база знаний + SQL
                ...
                Этот помощник отвечает по базе знаний (RAG) и строит корректные SQL‑запросы к вашей БД.

                **Как формулировать запросы**
                - Пишите обычным языком — ассистент сам выберет режим (RAG или SQL).
                - Уточняйте период явно: `2025-08-01 — 2025-08-31` или «за последние 30 дней».
                - Формулировки «топ/топ N/наиболее/наименьшие/ранжируй…» по умолчанию трактуются как задачи агрегирования:
                используется `GROUP BY` по указанным измерениям, агрегирование метрики (COUNT/COUNT DISTINCT/SUM/AVG — по смыслу),
                сортировка по метрике и `LIMIT N` (если N указан). Если метрика не названа — считаем количество объектов
                (`COUNT DISTINCT` ключевого идентификатора). Для «новые за период» — фильтр по дате создания (`created_at`/`registration_date`).

                **Что будет в ответе**
                - Заголовок «Таблица N: …» перед каждой таблицей.
                - Под таблицей — краткое пояснение (источники/фильтры/период).
                - Скрываемый блок «Показать SQL» с исходным запросом.
                - Экспорт таблиц/графиков доступен в истории результатов.

                **Примеры**
                Запрос к базе знаний:
                ```
                Дай краткое резюме по метрике конверсии: определение, формула и источники данных.
                ```

                Простой SQL‑запрос:
                ```
                Сделай топ 10 городов по количеству новых пользователей за последние 30 дней.
            """,
