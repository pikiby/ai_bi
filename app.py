...
def is_followup_sql_edit(text: str) -> bool:
    """
    Распознаём правки/повторы прошлого SQL.
    Ловим русские разговорные формулировки: «как прошлый», «на основе предыдущего», «добавь/измени/сортируй/фильтр», «топ/лимит» и т.п.
    """
    t = (text or "").lower().strip()
    t_compact = re.sub(r"[\s\-]+", "", t)

    keywords_any = [
        "на основе предыдущего запроса",
        "на основе прошлого запроса",
        "на основе последнего запроса",
        "как и прошлый",
        "как прошлый",
        "как предыдущий",
        "как последний",
        "повтори прошлый",
        "сделай такой же запрос",
        "такой же запрос",
        "правь прошлый",
        "измени прошлый",
        "добавь фильтр",
    ...
    # Слитные варианты типа «сделайтакойжезапрос», «последнегозапроса»
    keywords_compact = [
        "наосновепредыдущегозапроса",
        "наосновепрошлогозапроса",
        "наосновепоследнегозапроса",
        "такойжезапрос",
        "последнегозапроса",
    ]
    return any(k in t_compact for k in keywords_compact)
...
if mode == "sql":
    ...
    try:
        ...
        st.session_state.messages.append({
            "role": "assistant",
            "content": f"Использованный запрос:\n```sql\n{sql}\n```"
        })
        # 2) продублируем в кеш — get_last_sql() возьмёт отсюда быстрее, чем парсить историю
